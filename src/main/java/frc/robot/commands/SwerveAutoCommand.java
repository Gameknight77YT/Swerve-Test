...
public class SwerveAutoCommand extends CommandBase {
  ...
  private final double m_desiredRotation;
   ...
   * @param desiredRotation The angle in degrees that the drivetrain should be facing. This is sampled at each
   *     time step.
  ...
  public SwerveAutoCommand(
    ...
    this(
        ...
        yController,
        thetaController,
        trajectory.getStates().get(trajectory.getStates().size() - 1).poseMeters.getRotation().getDegrees(),
        outputModuleStates,
        requirements);
  }
  ...
  public void execute() {
    ...
    Rotation2d rotInput = Rotation2d.fromDegrees(curTime * m_desiredRotation / (m_trajectory.getTotalTimeSeconds()*.80));
    var desiredState = m_trajectory.sample(curTime);

    var targetChassisSpeeds =
        m_controller.calculate(m_pose.get(), desiredState, rotInput);
